...
export class LabEnvironment {
  ...
  private buildChaoticField() {
    ...
    arcs.forEach(({ radius, tube, pos, rot }) => {
      const arc = new THREE.Mesh(
        new THREE.TorusGeometry(radius, tube, 8, 64, Math.PI * 1.2),
        arcMaterial,
      );
  ...
  update(delta: number, elapsed: number) {
    this.floaters.forEach((element, index) => {
      const floatOffset =
        Math.sin(elapsed * element.floatSpeed + element.phase) * element.floatAmplitude;
      element.mesh.position.y = element.basePosition.y + floatOffset;
      element.mesh.rotateOnAxis(
        element.axis,
        delta * element.rotationSpeed * (index % 2 === 0 ? 1 : -1),
      );
    });
