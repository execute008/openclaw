...
export class HallsDataProvider {
    ...
    return jobs.map((job) => {
      ...
      return {
        ...
        metrics: {
          ...
          successRate:
            job.state?.lastStatus === "ok" ? 1 : job.state?.lastStatus === "error" ? 0 : 0.5,
  ...
  private buildNotionMetadataFallback(
    ...
    if (!base.description && savedMetadata.description)
      fallback.description = savedMetadata.description;
    if (!base.customColor && savedMetadata.customColor)
      fallback.customColor = savedMetadata.customColor;
  ...
  private calculateBusinessMetrics(
    ...
    return {
      ...
      activeLeads:
        sheetsMetrics?.activeLeads ?? (sheetsLeads.length || Math.floor(sessions.length / 3)),
