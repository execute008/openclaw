  ...
  constructor(options: HallsSceneOptions) {
    ...
    this.vrInput = new VRControllerInput({
      ...
      onGrabEnd: (station, startPosition, endPosition) => {
        ...
        hallsDataProvider
          .updateProjectPosition(station.getProject().id, endPosition)
          .catch(console.error);
  ...
  private setupEventListeners() {
    ...
    this.dragControls.onDrag((event) => {
      if (event.type === "drag:end" && event.startPosition) {
        ...
        const moved =
          event.startPosition.x !== event.position.x || event.startPosition.z !== event.position.z;
        ...
        if (moved) {
          ...
          // Persist to gateway
          hallsDataProvider
            .updateProjectPosition(event.station.getProject().id, event.position)
            .catch(console.error);
    ...
    this.undoRedoManager.onUndoRedo((command, isUndo) => {
      if (command.type === "move") {
        ...
        if (station) {
          ...
          // Persist to gateway
          hallsDataProvider.updateProjectPosition(command.projectId, newPos).catch(console.error);
    ...
    if (this.selectedStation) {
      ...
      if (project.status === "active" && project.linkedAgents.length === 0) {
        suggestions.push({
          ...
          description:
            "Active projects can benefit from automated workflows. Link an agent or create an n8n workflow.",
          context: "project",
          priority: "medium",
          actions: [
      ...
      if (activeProjects.length > 0) {
        const avgEnergy =
          activeProjects.reduce((sum, p) => sum + p.energy, 0) / activeProjects.length;
        if (avgEnergy < 5) {
          suggestions.push({
