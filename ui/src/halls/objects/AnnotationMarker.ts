...
export class AnnotationMarker {
   ...
   */
  private renderLabelCanvas(context: CanvasRenderingContext2D, width: number, height: number) {
    // Clear canvas
    ...
    const statusColor = this.annotation.status === "resolved" ? "#71717a" : "#22d3ee";
  ...
  private createAuthorSprite(): THREE.Sprite {
    ...
    context.fillText(
      `${this.annotation.author} \u2022 ${timestamp}`,
      canvas.width / 2,
      canvas.height / 2,
    );
  ...
  setHovered(hovered: boolean) {
    ...
    const markerMaterial = this.markerMesh.material as THREE.MeshStandardMaterial;
    markerMaterial.emissiveIntensity = hovered
      ? 0.6
      : this.annotation.status === "resolved"
        ? 0.1
        : 0.3;
  }
