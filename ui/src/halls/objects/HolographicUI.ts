...
export class HolographicUI {
  ...
  private addCornerDecorations(parent: THREE.Group, halfWidth: number, halfHeight: number) {
    ...
    corners.forEach((corner, index) => {
      ...
      const vertices = new Float32Array([
        ...
        corner.y,
        0,
        corner.x + (index % 2 === 0 ? cornerSize : -cornerSize),
        corner.y,
        0,
        corner.x,
        corner.y,
        0,
        corner.x,
        corner.y + (index < 2 ? -cornerSize : cornerSize),
        0,
      ]);
  ...
  showProjectDetails(project: Project) {
    ...
    this.detailsPanel.position.set(project.position.x, project.position.y + 3, project.position.z);
    ...
    const panelActions = project.notionUrl ? [...this.actions, NOTION_ACTION] : this.actions;
