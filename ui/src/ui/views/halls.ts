...
const PROJECT_STATUS_ORDER: Array<Project["status"]> = ["active", "paused", "completed", "hunting"];
          ...
          case "project:action": {
            ...
            if (payload.action === "toggle" && payload.project) {
              ...
              hallsDataProvider
                ...
                .then(() =>
                  setHallsFeedback(
                    `Updated ${payload.project?.name ?? "project"} status.`,
                    "success",
                  ),
                )
                .catch((err) => {
            ...
            if (payload.action === "edit" && payload.project) {
              ...
              hallsDataProvider
                ...
                .then(() =>
                  setHallsFeedback(
                    `Updated ${payload.project?.name ?? "project"} metadata.`,
                    "success",
                  ),
                )
                .catch((err) => {
                  const message =
                    err instanceof Error ? err.message : "Failed to update project metadata";
                  setHallsFeedback(message, "error");
                });
            }
